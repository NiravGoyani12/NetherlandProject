...
When(/in country switch flyout I ensure ([^\s]+) is interactable/, async (elementPath: string) => {
...
Then(/in country switch flyout I see country option list is ordered alphabetically/, async () => {
  const countryDropdown = await services.pageObject.getSelector('Experience.CountrySwitchFlyout.CountryDropdown') as DropdownSelector;
...
Then(/in country switch flyout I see country option (.*) contains text ([^\s]+)/, async (countryOption: string, text: string) => {
  const countryDropdown = await services.pageObject.getSelector('Experience.CountrySwitchFlyout.CountryDropdown', [countryOption]) as DropdownSelector;
