...
Then(/in shopping bag page I see product item ([^\s]+) has correct price info/, async (skuPartNumber: string) => {
  ...
  if (productItemDetail.WASPRICE) {
    ...
    commands.push(
      maybeDisplayed$(itemCurrentPriceSelector).then(async (ele) => {
        ...
        if (text.replace(/[\u00A0]/g, ' ') !== expectedCurrentPrice) {
          throw new Error(`The expect current price is ${expectedCurrentPrice}, but was ${text}`);
        }
      }),
      maybeDisplayed$(itemWasPriceSelector).then(async (ele) => {
        const text = await ele.getText();
        if (text.replace(/[\u00A0]/g, ' ') !== wasPrice) {
          throw new Error(`The expect WAS price is ${expectedCurrentPrice}, but was ${text}`);
        }
      }),
    );
  } else {
    ...
    commands.push(
      maybeDisplayed$(itemPriceSelector).then(async (ele) => {
        const text = await ele.getText();
        if (text.replace(/[\u00A0]/g, ' ') !== expectedCurrentPrice) {
          throw new Error(`The expect price is ${expectedCurrentPrice}, but was ${text}`);
        }
      }),
    );
  }
  ...
  results.forEach((r) => {
    expect(r, 'some elements are not displayed in Shopping bag page').not.to.be.null;
  });
});
